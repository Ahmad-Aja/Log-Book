...
import {
  ...
  CreateMedicalCaseFormInput,
} from "@/lib/validations/medical-case.schema";
// CreateMedicalCaseFormInput is used as the TFieldValues generic in useForm
...
export function AddMedicalCaseModal({
  isOpen,
  onClose,
}: AddMedicalCaseModalProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateMedicalCaseFormInput, unknown, CreateMedicalCaseFormData>({
    resolver: zodResolver(createMedicalCaseSchema(tValidation)),
    defaultValues: { title: "", description: "", caseDate: "", hospital: "" },
  });
