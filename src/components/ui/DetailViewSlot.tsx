...
export function DetailViewSlot({
  ...
  const baseClasses = multiline
    ...
    : "group p-2 sm:p-3 border border-gray-300 transition-all duration-200 flex flex-col sm:flex-row sm:items-center gap-1";
  ...
  const heightClasses = multiline
    ...
    : "min-h-[36px] sm:min-h-[44px] sm:h-[44px]";
  ...
  return (
    ...
    >
      <span
        className={`text-xs sm:text-sm font-medium whitespace-nowrap ${isDanger ? "text-red-600" : "text-wheat"}`}
      >
        {label}:
      </span>
      <span
        className={`text-sm sm:text-base min-w-0 flex-1 ${
          multiline
